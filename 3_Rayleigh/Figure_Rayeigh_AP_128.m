...
figure(1);
hold on; box on;
plot(sort(reshape(SE_CF_MMSE_tot_4_32,[K*nbrOfSetups,1])), linspace(0,1,K*nbrOfSetups),'r-','LineWidth',2);
plot(sort(reshape(SE_CF_MMSE_tot_16_8,[K*nbrOfSetups,1])), linspace(0,1,K*nbrOfSetups),'b-','LineWidth',2);
plot(sort(reshape(SE_CF_MMSE_tot_64_2,[K*nbrOfSetups,1])), linspace(0,1,K*nbrOfSetups),'m-','LineWidth',2);
...
figure(2)
hold on; box on;
plot(sort(SE_CF_MMSE_sum_4_32), linspace(0,1,nbrOfSetups),'r-','LineWidth',2);
plot(sort(SE_CF_MMSE_sum_16_8), linspace(0,1,nbrOfSetups),'b-','LineWidth',2);
plot(sort(SE_CF_MMSE_sum_64_2), linspace(0,1,nbrOfSetups),'m-','LineWidth',2);
xlabel('Sum of Spectral Efficency[bit/s/Hz]');
ylabel('CDF');
legend('M=4,N=32','M=16,N=8','M=64,N=2');