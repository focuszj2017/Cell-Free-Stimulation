...
%% Inital parameters
K = 16; %number of terminals
M = 64; %number of access points in cell free
Na = 1; %number of antennas per AP in cell free
M_cl = 1;%number of APs in collocated massive MIMO
Na_cl = 64;%number of antennas per AP in collocated
...
R_cf_MR_all = zeros(K,N);
R_cf_MMSE_all = zeros(K,N);
R_cl_MR_all = zeros(K,N);
R_cl_MMSE_all = zeros(K,N);
...
parfor n=1:N
    ...
    %compute MMSE combining
    V_MMSE = ((gain*diagP*gain')+eyeMNa)\(gain*diagP); %Matrix M_clNa_cl * K
    %compute SINR
    SINR = zeros(1,K);
    R_cl = zeros(1,K);
    for k = 1:K
...
Y2=linspace(0,1,N*K);
hold on;
plot(sort(R_cf_MR_all(:)), Y2(:), 'r--', 'LineWidth',2);
plot(sort(R_cf_MMSE_all(:)), Y2(:), 'r-', 'LineWidth',2);
plot(sort(R_cl_MR_all(:)), Y2(:), 'b--', 'LineWidth',2);
plot(sort(R_cl_MMSE_all(:)), Y2(:), 'b-', 'LineWidth',2);
xlabel('频谱效率 [bit/s/Hz]','FontSize',13);
ylabel('累计分布函数','FontSize',13);
legend('分布式MIMO(MRC)','分布式MIMO(MMSE)','集中式(MRC)','集中式(MMSE)');
